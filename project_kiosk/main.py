...
#   - 일자: 2023년10월13일
#   - 작성자: pilyunseon16
#   - 내용: 카페 음료를 주문 및 판매한느 콘솔 프로그램

# 조건
# 1.사용자는 최대 음료 1개, 베이커리 1개, 굿즈1개 구매가능!
from servise_kiosk import user_choice
# 메뉴와 가격표
#   -Dict Type -> 데이터 베이스
main_name = {1: "음료(Drink)" ,2: "빵(Bakery)",3:"굿즈"}
...
if choice == 1: # 음료
    print("&$ 음료(Drink) 메뉴")
    for i in range(len(main_name)):
        print(f"&$  {i+1}.{drink_name[i+1]} 메뉴({drink_price[i+1]})")
    # 4.세부 메뉴 선택
    ...
    print("&$ 빵(Bakery) 메뉴")
    for i in range(len(main_name)):
        print(f"&$  {i + 1}.{bakery_name[i + 1]} 메뉴({bakery_price[i + 1]})")
    # 4.세부 메뉴 선택
    ...
    print("&$ 굿즈(Goods) 메뉴")
    for i in range(len(main_name)):
        print(f"&$  {i + 1}.{goods_name[i + 1]} 메뉴({goods_price[i + 1]})")
    # 4.세부 메뉴 선택
