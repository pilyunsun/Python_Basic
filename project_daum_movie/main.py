...
for item in review_list:
    ...
    review_content = item.select("p.desc_txt")[0].get_text().strip() # .strip() = 좌우여백 제거
    review_content = re.sub("\n","",review_content)
    # \n : 한줄 개행
    # 수집한 리뷰가 개행 -> 문자열 안에 \n 포함
    print(f"  - 리뷰: {review_content}")
    review_writer = item.select("a.link_nick>span")[1].get_text() # [댓글 작성자, 작성자, 댓글 모아두기]
    print(f"  - 작성자: {review_writer}")

    # 24시간 내에 작성된 글은 날짜 -> 예: 21시간 전 ,17시간전
    ...
    if len(review_date) < 7:
        # 현재시간 - 17시간
        # 예:) 17시간전 -> 숫자만 추출: 17
        reg_hour = int(re.sub(r"[^~0-9]", "", total_review_cnt))
